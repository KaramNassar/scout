@props(['name', 'created_at', 'link', 'city', 'img', 'alt'])
    ...
    <div class="text-slate-500">
        <a href="{{ $link }}" class="block">
            <x-img :src="$img" :alt="$alt" class="rounded-bl-3xl rounded-tr-3xl sm:h-64 lg:h-72 transition duration-300 ease-in-out hover:scale-105"/>
            ...
            <div class="mt-4 flex items-center justify-center">
                <button
                    ...
                    <span
                        ...
                        <span
                            class="relative shrink-0 rounded-full border border-current bg-white px-3 py-2.5 text-indigo-600 active:text-indigo-500 transform transition-transform duration-300 ease-in-out hover:translate-x-1"
                        >
                             @if(app()->getLocale() === 'ar')
                                 &larr;
                             @else
