@props(['troop'])
    ...
    <div class="relative pl-8 sm:pl-32 pb-16 group">
    <div class="font-caveat font-medium text-2xl text-main-light dark:text-main-dark mb-1 sm:mb-0">{{ $troop->name }}</div>
    <div
        ...
        <div class="text-xl font-bold text-slate-900 dark:text-white/60">Was founded in {{ $troop->location }}</div>
    </div>
    <div class="text-slate-500">
        <a href="{{ route('troops.show', $troop->slug) }}" class="block">
            <x-img :src="$troop->getFirstMedia('troops')->getUrl()" :alt="$troop->name" class="rounded-bl-3xl rounded-tr-3xl sm:h-64 lg:h-72 transition duration-300 ease-in-out hover:scale-105"/>
